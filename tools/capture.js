...
client.once('clientReady', async () => {
  ...
  if (!guild) {
    console.error(
      `guild ${JSON.stringify(guildName)} not found. visible:`,
  ...
  if (!channel) {
    console.error(
      `voice channel ${JSON.stringify(channelName)} not found. visible:`,
...
function flush() {
  ...
  for (const [userId, chunks] of buffers) {
    ...
    if (data.length === 0) {
      console.log(`  (no audio for ${userId})`);
      continue;
    }
  ...
  if (!wrote)
    console.log(
      'NOTHING CAPTURED — check selfDeaf:false, that you actually spoke, and bot permissions',
    );
